# Authors: Initially created by Pierre Dragicevic and updated by Theophanis Tsandilas
...
# More appropriate for between independent groups CI estimation where R is the number of samples to create
# Generates the jackknife samples for rating data, assuming a fixed set of items
# and an infinite population of raters.
# Be careful: Fanis' implementation - not sure it works correctly!!!!!
jackknife.samples.random.between.raters <- function(ratings, R) {
    ...
    jackknife_samples <- list()
    for (i in 1:R) {
        # choose a random rater to remove
        r <- sample(1:n, 1)
        # remove all data from rater r
        ratings_subset <- ratings[,-r]
        # add the dataset to the list of samples
        jackknife_samples[[i]] <- ratings_subset
    }
...
#
# Generates the jackknife samples for rating data, assuming a fixed set of raters
# and an infinite population of items.
#
# This is for testing purposes as we don't need this in HCI.
#
jackknife.samples.random.items <- function(ratings) {
  
  # sample size for the jackknife = number of items
  n <- nrow(ratings)
  
  # generate jacknife samples
  jackknife_samples <- list()
  for (s in 1:n) {
    # remove all data from item s
    ratings_subset <- ratings[-s,]
    # add the dataset to the list of jackknife sample
    jackknife_samples[[s]] <- ratings_subset
  }
