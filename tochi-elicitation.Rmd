...
<div style="background-color:#F8F8FF;padding:26px;padding-top:1px;text-align:justify; outline-style:solid; outline-width:1px";>
##Abstract
...
##Materials
The article will appear at the <i>ACM Transanctions on Computer-Human Interaction (TOCHI)</i>, but you can download a [[PDF; 2.9MB]](https://www.lri.fr/~fanis/docs/gelicitation.pdf) preprint, author version of the article (50 pages).

Results from additional experiments can be downloaded as suplementary material [[PDF; 128KB]](tochi-supplementary.pdf).
You can also download the R code [[ZIP; 112KB]](R Code.zip) that I used for the analyses reported in the paper.

The article was partly based on a research report that I co-authored with Pierre Dragicevic back in September 2015:
...
**Note:** Pierre's contribution to this work has been significant and includes writing, developing R code, and engaging in long discussions. I encourage the readers to check his page on [statistical communication for HCI](http://www.aviz.fr/badstats). 
...
##A Critical Stance on a Controversial Topic
The article questions data-analysis methods that can be considered as mainstream in gesture-elicitation research. The agreement index *A* introduced by @Wobbrock:2005 has been used for the analysis of dozens, or maybe hundreds, of gesture elicitations studies in the past. However, the article shows that it can mislead researchers about how participants agree on their gesture assignments. In addition, the article identifies severe flaws in two papers recently published at CHI [@Vatavu:2015;@Vatavu:2016], where the first was awarded an honorable mention. My article's recommendation is that the significance tests, software, and agreement-interpretation guidelines introduced by this work should be abandoned. 
...
In my analysis, I distinguish between the physical gestures performed by participants and their signs. A sign can be thought of as the interpretation of an observed gesture, or otherwise, as a category that groups together ”equal” or ”similar” gestures. The classification observed gestures into signs is usually performed by humans.
...
##Major Issues
I try here to illustrate some of the isses raised by the article. 
...
@Vatavu:2016 included a short discussion about chance-corrected coefficients and concluded that such statistics are not appropriate for gesture elicitation methodologies. Their argument is that gestural vocabularies are open-ended.  Participants choose from a infinite number of possibilities, thus the problem of chance agreement is non-existent. 
The TOCHI article extensively discusses their argumentation and rejects their conclusion. 

###Bias and Its Effect on Agreement
In practice, agreement is not assessed over an infinite set of gesture possibilities. Participants’ gesture proposals are first classified into signs, and agreement is assessed over the sign vocabulary defined by that specific classification process. However, the article shows that even if we assumed an infinite number of signs. The reason is the presence of <emph>bias</emph>, an overall tendency of some signs to appear more frequently than others, independently of the referents. The article discusses usability, social, legacy, procedural, and classification biases, which can cause a small number of signs to dominate the results of a gesture elicitation study.

Bias increases the likelihood of chance agreement and makes the use of the AR index problematic. The interactive figures below demonstrate how bias affects agreement scores under a simulation setting, when participants are blindfolded so they cannot see the referents for which they propose gestures. Fleiss' Kappa always gives values that are very close to zero, but AR values increase as bias becomes stronger. 
 ...
 ui <- fluidPage(
  ...
  output$simPlot <- renderPlot({
    ...
    if(input$distribution == 1){
      isolate(
        vals$ar <- simulation(vals$ar, participants, referents, zipf, B, percent.agreement)
      )
      isolate(
      ...
      isolate(
        vals$ar <- simulation(vals$ar, participants, referents, normal, sd, percent.agreement)
      )
    ...
    legend("topright", inset=.01, legend=c("Fleiss' Kappa", "Agreement Rate (AR)"), 
           fill=c(rgb(.86,.86,.95), rgb(.93,.8,.5)), horiz=FALSE, cex=1.1, box.lty=0)
...
We see that AR values also capture random agreements that are not specific to the referents that participants are exposed. Unfortunately, such agreements say nothing about how appropriate a given mapping between signs and referents is. Readers may still wonder whether such bias levels are realistic. The figure below shows bias distributions observed in four gesture elicitation studies published at CHI. We see that bias can be particulary high. What is more, it varies considerably across studies, which implies that comparisons based on agreement rates are problematic.  

<center>
![](bias-studies.png){ width=700px }
<div style="padding:36px;padding-top:8px;padding-bottom:0px;font-size:14px"><b style="color:sienna">Observed</b> bias distributions for our four case studies. We also show the <b style="color:DarkBlue">closest model</b> of bias distribution.</div>
</center>

###Interpreting Agreement Values
@Vatavu:2015 proposed some generic guidelines on how to interpret the magnitude of agreement: AR < .100 is low agreement, .100 < AR < .300 is medium, .300 < AR < .500 is high, and AR > .500 is very high agreement. These guidelines derive from two types of analysis: (1) a probabilistic reasoning, and (2) a survey of agreement rates from past studies. 

We showed earlier that AR can be misleading about the level of true agreement. For example, agreement rates for the study of @lahey2011 were AR = .251 (Session 2) and AR = .298 (Session 3). These values correspond to <b>medium agreement</b> according to @Vatavu:2015. However, my analysis shows a very low chance-corrected agreement for this study, where Fleiss' Kappa is equal to .040, 95% CI [-.064, .144] for Session 2, and .043, 95% CI [-.056, .141] for Session 3. Note that Fleiss' Kappa is not statistically significantly different than zero. Interestingly, this analysis confirms the conclusion of the authors who state: <i>"the consensus on the mapping of those bend gestures to actions was overall low, showing that each participant had his or her own preference.”</i> [@lahey2011]

Furthermore, the probabilistic reasoning of @Vatavu:2015 has several errors. The article further explains that even if one corrects these errors, relying on statistics to assess the magnitude of agreement is not feasible. To this end, we need other criteria, such as usability metrics, and longer-term studies.

###Statistical Inference
The third issue that the article investigates is how support statistical inference. Vatavu and Wobbrock have proposed two statistical tests to support hypothesis testing: (1) the Vrd statistic for comparing agreement rates within participants [@Vatavu:2015], and (2) the Vb statistic for comparing agreement rates between independent participant groups [@Vatavu:2016]. However, a careful analysis of these statistics shows that the probabilistic reasoning that the authors used to derive them has flaws. Their evaluation through Monte Carlo simulations further demonstrates that both statistics yield Type I error rates that are orders of magnitude higher than their nominal levels. 
...
shinyApp(
 ui <- fluidPage(
  ...
  server <- function(input, output) {
    ...
    # Initiate the reactive values here...
    vals <- reactiveValues(counter = 0, counter_ = 0, errors.vrd = rep(0, L), errors.vb = rep(0, L))
...
The evaluations reported in the article consider different distribution models to simulate participants' proposals. My analysis of four gesture elicitation studies shows that models based on <b>Zipf-Mandelbrot distributions</b> approximate well frequencies that emerge in real datasets.

##Remedies and Recommendations
Fortunately, other research disciplines have long established methods to support statistical inference for agreement indices [@gwet2014]. Those are based on interval estimation and resampling methods, in particular jackknifing [@quenouille1949] and bootstrapping [@efron1979]. The TOCHI article further evaluates them in the context of gesture elicitation and shows how to apply them to analyze real datasets. However, the article also highights limitations of such methods. For example, jackknife and bootstrap confidence intervals can not be precise when applied to individual referents, and sample sizes are small. Driving conclusions based on statistical significance is not a good practice in this case. 

Overall, not a single statistic is enough to capture the results of a gesture elicitation study. My recommendation is to report both raw percent agreement and chance-corrected agreement indices, as well as agreement indices specific to individual signs. A separate investigation of the observed bias distribution is also recommended for every gesture elicitation study. Finally, agreement scores have little value by themselves. In order to interpret them, one needs to know exactly what participants agree upon, i.e., know how signs are defined and how proposals are classified into these signs. It is therefore imperative that authors clarify how they define their sign vocabularies, how they classify observed gestures into these vocabularies, and how they verify the reliability of this classification process.

##Feedback and Comments
I am not an expert in statistics, and writing this artcile has been a learning process for me. I will appreciate any feedback on mistakes or other problems that you may discover in my analyses. I am also interested in comments and suggestions on how to improve or extend this work. 

Creating an R library for the the statistics recommended by the article is a future goal. For the moment, feel free to use the R code that is attached as supplementary material on this page. Please, contact me to indicate problems or ask questions about the code. 
